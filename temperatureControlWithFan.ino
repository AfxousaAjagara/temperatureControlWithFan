...
void getDeviceStatus(char* payload) {
  ...
  // Read led status
  const char* fan = (fan1.getState() == FAN_OFF)? "ON" : "OFF";

  // make payload for the device update topic ($aws/things/MyMKRWiFi1010/shadow/update)
  sprintf(payload,"{\"state\":{\"reported\":{\"temperature\":\"%0.2f\",\"FAN\":\"%s\"}}}",t,fan);
}
...
void onMessageReceived(int messageSize) {
  ...
  const char* fan = state["FAN"];
  float temperature_num = dht.readTemperature();
  Serial.println(fan);
  
  char payload[512];
  if (strcmp(fan,"ON")==0||temperature_num>=30) {
    fan1.off();
